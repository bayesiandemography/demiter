...
## 'self' is a component in a Movements
## account. If the account has age, then there are two 'oth'
## arrays with identical dimensions: the first array describes
## increments affecting accession, and the second array describes
## increments affecting population at the end of the period.
## If the account does not have age, then there is one 'oth'
## array.


## Assume that if 'self' shares a dimension
## with 'oth', the two dimensions have exactly
## the same levels, in the same order,
## with the partial exception of time.
## The time dimension for 'self' has intervals
## (t, t+n), (t+n, t+2n), ... (t+(k-1)n, t+kn),
## while the time dimension for 'oth' has points
## t+n, t+2n, ..., t+kn.
...
iter_next_increment <- function(iter) {
    ...
    if (has_triangle) {
        is_upper <- pos_triangle_self[[i_triangle_self]] == 2L
        index_array_oth <- if (is_upper) 1L else 2L
    }
    else
        index_array_oth <- 0L
    i_oth <- c(index_array_oth, 0L, 0L)
    i_oth_base <- 1L
    ...
    if (i_comp_type_self == 1L) # increment
        ...
        for (i_orig_dest in seq_len(n_orig_dest_self)) {
            ...
            offsets <- offsets - (pos_orig - 1L) * stride_orig + (pos_dest - 1L) * stride_dest
        }
        i_oth[[2L]] <- i_oth_base + offsets
        i_oth[[3L]] <- i_oth_base
    }
